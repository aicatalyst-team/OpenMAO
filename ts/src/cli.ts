...
export async function runCli(args: string[], options: CliOptions = {}): Promise<number> {
  ...
  try {
    ...
    if (command === "help" || command === "--help" || command === "-h") {
      write(
        "openmao demo | demo-approve | init | run demo|resume | worker demo|demo-approve | work list|show|create|assign|status|envelope|outcome|review | workers list|register | ingest list|record | learning scan|proposals|show|apply | cos init|tick|inbox|read <id> [--unread] [--at ts] | cadence list|add --kind <kind> --interval <seconds> | approvals list|approve|reject <id> [--workspace workspace_id] | events [run_id]|--workspace [workspace_id] | world [--run run_id] [--workspace workspace_id] | console",
      );
      return 0;
    }
    if (command === "init") {
